...
asterisk:
  ...
  dongle: gsm
  ring_wait_seconds: 24           # секунд звонка перед автоответчиком
...
2. Через 24 секунды (или раньше, если вы не отвечает) — подключается автоответчик
...
|---|---|---|---|
| Длительность гудков | `asterisk.ring_wait_seconds` | 24 сек | Сколько гудеть перед автоответчиком |
...
2. Если модем не отвечает — инкрементирует счётчик
3. После 3 неудачных проверок — пытается сбросить модем
4. Если сброс не помог — логирует ошибку
...
2. На GSM-узле: `node.role: gsm`, `agent.listen: "<Tailscale-IP>:8090"`
3. На Telegram-узле: `node.role: telegram`, `agent.allowed_peers: ["<Tailscale-IP-GSM>"]`
4. В `agent.listen` и `userbot_http.listen` — IP Tailscale, НЕ 127.0.0.1
