...
| `/etc/simbridge/modem-profiles/` + `active_modem` | Профили модемов (смена модема одной командой) | 0644 |
...
asterisk:
  ...
  dongle: gsm
  ring_wait_seconds: 18           # секунд звонка перед автоответчиком
...
### 3.7 Профили модемов — смена модема одной командой

Все модем-специфичные настройки (udev-правила, `dongle.conf`, два ключа
`simbridge.yaml`, `MODEM_ID`) хранятся в **профиле** — одном YAML-файле на
модем. Смена модема = одна команда `use <профиль>`: скрипт сделает бэкапы,
перерендерит все четыре артефакта, обновит конфиг, перезапустит нужные
сервисы и проверит регистрацию. Секреты, ACL, чёрный список, Tailscale и
dialplan не трогаются.

```bash
MP="python3 /opt/simbridge/scripts/modem_profile.py"

$MP status            # что вставлено (USB) vs активный профиль, статус dongle
$MP list              # все профили, активный, verified/UNVERIFIED

# Сохранить ТЕКУЩИЙ работающий модем в профиль (до вынимания!):
$MP capture huawei-e161

# Новый модем: осмотр портов и фиксация результата в профиль:
$MP probe
$MP probe --commit quectel-ec25 --data /dev/ttyUSB2 --audio /dev/ttyUSB0

# Переключение (unverified-профиль требует --force):
$MP use quectel-ec25 --force

# Откат всегда — вставить старый модем и:
$MP use huawei-e161
```

Гарантии: идемпотентность (повторный `use` активного профиля — no-op,
без рестартов), бэкапы всех артефактов при каждой смене, unverified-профиль
отказывается без `--force`. Подробности и полный workflow —
[`docs/modem-profiles.md`](modem-profiles.md).
...
2. Через 10 секунд (или раньше, если вы не отвечает) — подключается автоответчик
...
|---|---|---|---|
| Длительность гудков | `asterisk.ring_wait_seconds` | 18 сек | Сколько гудеть перед автоответчиком |
